...
type Project={id:string;contractId?:string;name:string;category:string;description:string;agency:string;contractor?:string;budget?:number;amountPaid?:number;status:string;progress?:number;location:string;region?:string;districtOffice?:string;programName?:string;infrastructureYear?:string;startDate?:string;completionDate?:string;sourceOfFunds?:string;livestreamUrl?:string;sourceRevision?:string;sourceImportedAt?:string;sourceUrl:string;lastChecked:string};
...
const money=(value?:number)=>value==null?'Not reported':new Intl.NumberFormat('en-PH',{style:'currency',currency:'PHP',maximumFractionDigits:0}).format(value);
const shortDate=(value?:string)=>value?new Date(value).toLocaleDateString('en-PH'):undefined;
